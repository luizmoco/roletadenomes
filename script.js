...
const nomesPadrao = ['Rafael', 'Italo', 'Flavio', 'Henrique', 'Marcos', 'Vandrei', 'Anderson', 'Thaina', 'Roberson', 'Peterson', 'Victor', 'Helio'];
const nomesFerias = ['Luiz :('];
const licenca = [];
const ausenciaSegundaFeira = ['Raphael'];
...
function criarCheckboxes() {
  ...
  if (diaSemana === 1) {
    ausenciaSegundaFeira.forEach(nome => {
      ...
      label.appendChild(checkbox);
      label.appendChild(document.createTextNode(nome + " Treinamento"));
      containerCheck.appendChild(label);
    });
  }
